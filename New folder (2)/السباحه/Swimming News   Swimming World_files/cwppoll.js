...
function vote_poll(pollid, answertype, maxnoanswers){
    ...
    if(answertype == 'multiple')
    {
    	...
    	//MFI_Customization_Start // allow from outside
    	if (data == null || data == "")
    	{
    		data = jQuery(document.forms[0]).serialize();
    	}
    
		//MFI_Customization_End
    ...
    if(answertype == 'one')
    {
        ...
        jQuery.post(ajaxurl, data,
            ...
            {
            	debugger;
            	//MFI_Customization_Start
            	jQuery('#weeklypollform' + pollid).html(response);
            	//jQuery('#poll'+pollid).html(response);
            	//Query('#pollsc'+pollid).html(response);
            	//MFI_Customization_End
            }
        ); 
    }
